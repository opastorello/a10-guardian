...
from a10_guardian.core.dependencies import get_mitigation_service, get_system_service
...
class TestApiEndpoints:
    ...
    def test_get_zone_status_found(self, client, mock_mitigation_service):
        ...
        response = client.get("/api/v1/mitigation/zones/status/1.2.3.4", headers=AUTH)
        assert response.status_code == 200
    ...
    def test_get_zone_status_not_found(self, client, mock_mitigation_service):
        ...
        response = client.get("/api/v1/mitigation/zones/status/9.9.9.9", headers=AUTH)
    ...
    def test_remove_zone(self, client, mock_mitigation_service):
        ...
        response = client.delete("/api/v1/mitigation/zones/remove/1.2.3.4", headers=AUTH)
        assert response.status_code == 200
    ...
    def test_list_zones(self, client, mock_mitigation_service):
        ...
        response = client.get("/api/v1/mitigation/zones/list", headers=AUTH)
        assert response.status_code == 200
    ...
    def test_get_system_info(self, client, mock_system_service):
        ...
        response = client.get("/api/v1/system/info", headers=AUTH)
        assert response.status_code == 200
